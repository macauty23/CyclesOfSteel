...
export const WEAPON_TECH_TREE: Record<WeaponTechNodeId, WeaponTechDefinition> = {
  ...
  steelDusack: {
    ...
    swordId: "steelDusack",
    name: "Steel Dusack",
    shortName: "Steel Dusack",
    summary: "A hardened drill blade whose clean moving sweeps turn into a brief burst of passing footwork.",
    detail: "Passing Cut: land a clean sweep while moving to gain a short movement-speed boost. Retains Rolling Edge.",
    accent: 0xd8a66a,
    cost: { steel: 6, wood: 1, leather: 3, bamboo: 2, gemstone: 1 },
    prerequisiteIds: ["dusack"],
    position: { x: 1260, y: 1200 },
    bonuses: { moveSpeed: 5, lightRange: 5, sweepWidth: 8 }
  },
  mastersDusack: {
    id: "mastersDusack",
    branch: "broad",
    swordId: "mastersDusack",
    name: "Master's Dusack",
    shortName: "Master's Dus.",
    summary: "A final training blade that turns a Defensive Bind into one especially long, forceful redirection sweep.",
    detail: "Redirection: after a Defensive Bind, your next sweep gains reach and knockback. It is a controlled answer, not a damage steroid.",
    accent: 0xf0cf8c,
    cost: { steel: 10, wood: 2, leather: 4, bamboo: 2, gemstone: 2, amber: 1, essence: 1 },
    prerequisiteIds: ["steelDusack"],
    position: { x: 1440, y: 1290 },
    bonuses: { moveSpeed: 6, lightRecoveryScale: 0.92, sweepRange: 8, sweepKnockback: 26 }
  },
  officersCutlass: {
    id: "officersCutlass",
    branch: "broad",
    swordId: "officersCutlass",
    name: "Officer's Cutlass",
    shortName: "Officer's Cut.",
    summary: "A final boarding blade whose third clean sweep carries commanding impact and accelerates Flow.",
    detail: "Commanding Edge: every third clean sweep gains impact and an extra Flow contribution. Whiffs never advance the count.",
    accent: 0xf5cd86,
    cost: { steel: 13, wood: 3, leather: 4, coral: 3, stormglass: 2, gemstone: 2, essence: 1 },
    prerequisiteIds: ["navalCutlass"],
    position: { x: 1800, y: 450 },
    bonuses: { moveSpeed: 6, dashCooldown: -16, sweepDamage: 3, sweepKnockback: 24 }
  },
  executionFalchion: {
    id: "executionFalchion",
    branch: "broad",
    swordId: "executionFalchion",
    name: "Execution Falchion",
    shortName: "Execution Fal.",
    summary: "A final cleaver that saves its highest damage for staggered, stunned, or guard-broken enemies.",
    detail: "Headsman: heavy attacks deal substantially more damage only against a genuine opening.",
    accent: 0xd47b59,
    cost: { steel: 12, wood: 3, leather: 4, obsidian: 3, gemstone: 2, brimstone: 1, essence: 2 },
    prerequisiteIds: ["greatFalchion"],
    position: { x: 1440, y: 810 },
    bonuses: { maxHp: 5, heavyDamage: 5, heavyKnockback: 36, sweepWidth: 10 }
  },
  warFalchion: {
    id: "warFalchion",
    branch: "broad",
    ...
    name: "War Falchion",
    shortName: "War Fal.",
    summary: "A final war cleaver that makes charged heavy cuts the answer to armor and an active enemy guard.",
    detail: "Sundering Edge: charged heavies gain impact, improve armor effectiveness, and collapse a currently held enemy guard.",
    accent: 0xc56949,
    cost: { steel: 13, wood: 3, leather: 4, obsidian: 3, brimstone: 2, gemstone: 2, essence: 1 },
    prerequisiteIds: ["greatFalchion"],
    position: { x: 1440, y: 990 },
    bonuses: { maxHp: 7, heavyDamage: 5, heavyKnockback: 44, sweepWidth: 12 }
  },
  hauswehr: {
    id: "hauswehr",
    branch: "messer",
  ...
  heavyKriegsmesser: {
    ...
    branch: "messer",
    swordId: "heavyKriegsmesser",
    name: "Heavy Kriegsmesser",
    shortName: "Heavy Krieg.",
    summary: "A brutal Grosses Messer fork whose charged heavy rewards entering the cut with real stamina reserves.",
    detail: "Brutal Commitment: charged heavy damage scales with committed stamina, capped so it cannot become an infinite-resource payoff.",
    accent: 0xb7785f,
    cost: { steel: 10, wood: 2, leather: 4, obsidian: 2, gemstone: 2, brimstone: 1 },
    prerequisiteIds: ["grossesMesser"],
    position: { x: -360, y: 840 },
    bonuses: { maxHp: 4, heavyDamage: 5, heavyKnockback: 32 }
  },
  executionMesser: {
    id: "executionMesser",
    branch: "messer",
    swordId: "executionMesser",
    name: "Execution Messer",
    shortName: "Execution Mess.",
    summary: "A final brutal messer that turns a real Guard Break into a short, violent execution opportunity.",
    detail: "Sentence: attacks against a stunned or guard-broken target deal a large bonus and carry a sharp impact frame. No instant kill.",
    accent: 0xd19272,
    cost: { steel: 13, wood: 3, leather: 4, obsidian: 3, gemstone: 2, brimstone: 2, essence: 1 },
    prerequisiteIds: ["heavyKriegsmesser"],
    position: { x: -540, y: 750 },
    bonuses: { maxHp: 5, heavyDamage: 6, heavyKnockback: 40, sweepWidth: 10 }
  },
  warMesser: {
    id: "warMesser",
    branch: "messer",
    swordId: "warMesser",
    name: "War Messer",
    shortName: "War Messer",
    summary: "A kriegsmesser fork built around moving forward, landing clean, and getting back to the next threat sooner.",
    detail: "Forward Pressure: clean attacks while moving toward the enemy reduce their recovery instead of adding flat damage.",
    accent: 0xae8067,
    cost: { steel: 8, wood: 2, leather: 4, obsidian: 2, gemstone: 1, amber: 1 },
    prerequisiteIds: ["kriegsmesser"],
    position: { x: -180, y: 1200 },
    bonuses: { moveSpeed: 8, moveAcceleration: 70, lightDamage: 3, heavyDamage: 3 }
  },
  feldmesser: {
    id: "feldmesser",
    branch: "messer",
    swordId: "feldmesser",
    name: "Feldmesser",
    shortName: "Feldmesser",
    summary: "A campaign continuation that can keep one earned Flow chain from collapsing immediately when the player takes damage.",
    detail: "Campaigner: once per encounter, a damaging hit preserves Flow briefly. The damage and other consequences still apply.",
    accent: 0xc69a78,
    cost: { steel: 10, wood: 2, leather: 4, obsidian: 2, amber: 2, gemstone: 2, essence: 1 },
    prerequisiteIds: ["warMesser"],
    position: { x: -360, y: 1290 },
    bonuses: { maxHp: 4, moveSpeed: 6, comboWindow: 140, heavyDamage: 3 }
  },
  landsknechtMesser: {
    id: "landsknechtMesser",
    branch: "messer",
    swordId: "landsknechtMesser",
    name: "Landsknecht Messer",
    shortName: "Landsknecht",
    summary: "A final pressure messer that earns movement only while consecutive clean attacks genuinely keep the opponent under threat.",
    detail: "Relentless: clean pressure stacks short movement boosts; true neutral, damage, and encounter end clear it.",
    accent: 0xe1bd8d,
    cost: { steel: 13, wood: 3, leather: 5, obsidian: 3, amber: 2, gemstone: 2, brimstone: 1, essence: 1 },
    prerequisiteIds: ["feldmesser"],
    position: { x: -540, y: 1380 },
    bonuses: { maxHp: 5, moveSpeed: 8, moveAcceleration: 90, lightDamage: 3, heavyDamage: 4 }
  },
  fechtmesser: {
    id: "fechtmesser",
    branch: "messer",
    swordId: "fechtmesser",
    name: "Fechtmesser",
    shortName: "Fechtmesser",
    summary: "A technical Messer fork whose Standard Bind makes the next light countercut more dangerous.",
    detail: "Countercut: use the safe Standard Bind, then land the next light attack before the small window expires.",
    accent: 0xa7c0af,
    cost: { steel: 5, wood: 1, leather: 3, bamboo: 2, gemstone: 1 },
    prerequisiteIds: ["messer"],
    position: { x: 0, y: 750 },
    bonuses: { moveSpeed: 7, lightDamage: 3, lightWindupScale: 0.92, parryWindow: 8 }
  },
  longFechtmesser: {
    id: "longFechtmesser",
    branch: "messer",
    swordId: "longFechtmesser",
    name: "Long Fechtmesser",
    shortName: "Long Fecht.",
    summary: "A longer fencing messer that turns a Perfect Bind into a very short reduced-windup opportunity.",
    detail: "Indes: Perfect Bind, then take the initiative before the timing window closes.",
    accent: 0xc4d9c7,
    cost: { steel: 8, wood: 2, leather: 4, bamboo: 2, gemstone: 2, amber: 1 },
    prerequisiteIds: ["fechtmesser"],
    position: { x: -180, y: 570 },
    bonuses: { moveSpeed: 6, lightRange: 7, thrustRange: 8, parryWindow: 10 }
  },
  mastersMesser: {
    id: "mastersMesser",
    branch: "messer",
    swordId: "mastersMesser",
    name: "Master's Messer",
    shortName: "Master's Mess.",
    summary: "A final technical messer that turns exact measure into one more forceful, quicker initiative answer.",
    detail: "Vor: a perfect-measure hit arms the next attack with better recovery and impact, rewarding deliberate line control.",
    accent: 0xe0efcf,
    cost: { steel: 11, wood: 2, leather: 4, bamboo: 2, amber: 2, gemstone: 2, essence: 2 },
    prerequisiteIds: ["longFechtmesser"],
    position: { x: -360, y: 480 },
    bonuses: { moveSpeed: 7, lightDamage: 3, thrustDamage: 3, comboWindow: 120, parryWindow: 12 }
  }
};
